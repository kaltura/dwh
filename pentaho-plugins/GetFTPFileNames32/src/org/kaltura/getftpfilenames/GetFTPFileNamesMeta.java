...
public class GetFTPFileNamesMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public String getXML()
	{
		...
		retval.append("    ").append(XMLHandler.addTagValue("usename", getUsername()));
	...
	private void readData(Node stepnode) throws KettleXMLException
	{
		try
		{
			...
			setActiveFtpConnectionMode("Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode, "activeFtpConnectionMode")));
	...
	public FTPFileInputList getFileList(FTPClient ftpClient, VariableSpace space)
	{
		return FTPFileInputList.createFileList(ftpClient, space, fileName, fileMask, fileRequired, buildSubdirsFlagsArray(), buildFileTypeFiltersArray());
	}
	public FTPFileInputList getDynamicFileList(FTPClient ftpClient,VariableSpace space, String[] filename, String[] filemask, String[] filerequired)
	{
		return FTPFileInputList.createFileList(ftpClient, space, filename, filemask, filerequired, buildSubdirsFlagsArray(), buildFileTypeFiltersArray());
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta, StepMeta stepinfo, RowMetaInterface prev, String input[], String output[], RowMetaInterface info)
	{
		...
		{
			if (input.length > 0)
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString("GetFTPFileNamesMeta.CheckResult.InputOk"), stepinfo);
			else
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, Messages.getString("GetFTPFileNamesMeta.CheckResult.InputErrorKo"), stepinfo);
			remarks.add(cr);
			
			if(Const.isEmpty(dynamicFilenameField))
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, Messages.getString("GetFTPFileNamesMeta.CheckResult.FolderFieldnameMissing"), stepinfo);
			else
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString("GetFTPFileNamesMeta.CheckResult.FolderFieldnameOk"), stepinfo);
			remarks.add(cr);
			
		}else{

			if (input.length > 0)
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, Messages.getString("GetFTPFileNamesMeta.CheckResult.NoInputError"), stepinfo);
			else
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString("GetFTPFileNamesMeta.CheckResult.NoInputOk"), stepinfo);
			...
			//	cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, Messages.getString("GetFTPFileNamesMeta.CheckResult.ExpectedFilesError"), stepinfo);
			//else
			//	cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString("GetFTPFileNamesMeta.CheckResult.ExpectedFilesOk", ""+fileList.nrOfFiles()), stepinfo);	
			//remarks.add(cr);
		}
	}
