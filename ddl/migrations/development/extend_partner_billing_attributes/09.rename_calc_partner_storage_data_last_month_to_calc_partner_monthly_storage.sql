...
CREATE DEFINER=`root`@`localhost` FUNCTION `calc_partner_monthly_storage`(month_id INT ,partner_id INT) RETURNS DECIMAL(19,4)
    DETERMINISTIC
BEGIN
	...
	FROM (		SELECT 	all_times.day_id date_id,
				IFNULL(IF(IF(aggr_p.bandwidth_source_id=1,1,0) = 1, aggr_p.aggr_storage_mb,
