...
BEGIN
	...
	DECLARE v_group_column VARCHAR(50);
	DECLARE v_entity_table VARCHAR(50);
	DECLARE v_aggregation_phrase VARCHAR(50);
	DECLARE v_aggregation_table VARCHAR(50);
	DECLARE v_bridge_entity VARCHAR(50);
	DECLARE v_bridge_table VARCHAR(50);
	...
	SET @s = CONCAT('SELECT dim.', v_group_column,', ', v_aggregation_phrase, 
			...
			' = dim.', v_group_column, ' AND dim.operational_measures_updated_at BETWEEN p.date_value AND \'', v_execution_start_time, '\' AND p.id = ', v_last_execution_parameter_id);
	PREPARE stmt FROM  @s;
	EXECUTE stmt;
