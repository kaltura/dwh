...
BEGIN
	...
	UPDATE kalturadw.dwh_dim_kusers kusers, 
		(SELECT DISTINCT entries.kuser_id FROM kalturadw.dwh_dim_entries entries, kalturadw.dwh_fact_entries_sizes fact
			WHERE entries.entry_id = fact.entry_id AND fact.entry_size_date_id = p_date_id) updated_kusers
	SET kusers.operational_measures_updated_at = NOW()
	WHERE kusers.kuser_id = updated_kusers.kuser_id;
END$$
