...
BEGIN
	...
	/*Insert the unified api calls into the API Calls fact*/	
	...
	/*Insert the failed unified api calls into the errors fact*/	
	...
	/*Delete the partial calls that were just unified*/
	DELETE partial_calls
	FROM 	kalturadw.dwh_fact_incomplete_api_calls partial_calls, 
		unified_api_calls unified_calls
	WHERE 
		partial_calls.session_id = unified_calls.session_id
		AND partial_calls.request_index = unified_calls.request_index;
END$$
