...
BEGIN
	DELETE FROM kalturadw.dwh_hourly_partner_usage WHERE date_id = date(date_val)*1 and IFNULL(count_bandwidth_kb,0) > 0 and ifnull(count_storage_mb,0) = 0 and ifnull(aggr_storage_mb,0) = 0;
        UPDATE kalturadw.dwh_hourly_partner_usage SET count_bandwidth_kb = null WHERE date_id = date(date_val)*1 and (ifnull(count_storage_mb,0) > 0 or ifnull(aggr_storage_mb,0) > 0);
