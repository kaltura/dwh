...
BEGIN
    ...
    SELECT entry_id, categories, updated_at
    ...
    read_loop: LOOP
        FETCH entries INTO v_entry_id, v_categories, v_updated_at;
        IF done THEN
             LEAVE read_loop;
    END IF;
        ...
        IF v_categories IS NULL OR v_categories = '' THEN
            -- add the entry category
                    INSERT IGNORE INTO dwh_dim_entry_categories (entry_id, category_id, updated_at) VALUES (v_entry_id, 1, v_updated_at);
                    SET v_categories_done = 1;
        END IF;
        
        WHILE NOT v_categories_done DO
            SET v_category_name = SUBSTRING(v_categories, v_categories_idx,
                    IF(LOCATE(',', v_categories, v_categories_idx) > 0,
                    LOCATE(',', v_categories, v_categories_idx) - v_categories_idx,
                    LENGTH(v_categories)));

            SET v_category_name = TRIM(v_category_name);
            IF LENGTH(v_category_name) > 0 THEN
                ...
                INSERT IGNORE INTO dwh_dim_categories (category_name) VALUES (v_category_name);
                
                SELECT category_id INTO v_category_id FROM dwh_dim_categories WHERE category_name = v_category_name;

                -- add the entry category
                INSERT IGNORE INTO dwh_dim_entry_categories (entry_id, category_id, updated_at) VALUES (v_entry_id, v_category_id, v_updated_at);
            ELSE
                SET v_categories_done = 1;
            END IF;
        END WHILE;
