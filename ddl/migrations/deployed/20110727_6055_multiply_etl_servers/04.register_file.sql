...
CREATE DEFINER=`etl`@`localhost` PROCEDURE `register_file`(p_file_name VARCHAR(750), p_process_id INT, p_file_size_kb INT(11), p_compression_suffix VARCHAR(10))
BEGIN
	...
	IF (v_file_id IS NULL) THEN
		...
		IF (v_assigned_server_id IS NOT NULL) THEN
			...
			IF (v_cycle_id IS NOT NULL) THEN 
				INSERT INTO kalturadw_ds.files 	(file_name, file_status, insert_time, file_size_kb, process_id, cycle_id, compression_suffix)
							VALUES	(p_file_name, 'IN_CYCLE', NOW(), p_file_size_kb, p_process_id, v_cycle_id, p_compression_suffix);
			ELSE
