...
BEGIN
	...
	DECLARE staging_areas_cursor CURSOR FOR SELECT 	source_table, fact_table_name, IFNULL(on_duplicate_clause,''),	staging_partition_field, post_transfer_sp, aggr_date_field, hour_id_field, post_transfer_aggregations, reset_aggregations_min_date, ignore_duplicates_on_transfer
											FROM staging_areas s, cycles c, fact_tables f
											WHERE s.process_id=c.process_id AND c.cycle_id = p_cycle_id and s.target_table_id = f.fact_table_id;
