...
BEGIN
	...
	UPDATE aggr_managment SET is_calculated = 0, start_time = NOW() WHERE aggr_name = p_aggr_name AND aggr_day_int = DATE(p_date_val)*1;
	...
	UPDATE aggr_managment SET is_calculated = 1, end_time = NOW() WHERE aggr_name = p_aggr_name AND aggr_day_int = DATE(p_date_val)*1;
END$$
